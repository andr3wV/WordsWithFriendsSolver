  ...
  try{
    for(let i = 1; i < 226; i++) {
      ...
      await image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
        ...
        if (red === 65 && green === 33 && blue === 21 || red === 255 && green === 255 && blue === 255 && alpha === 255) {
        ...
        image.resize(Jimp.AUTO, 0.3 * image.bitmap.height)
        .grayscale()
        .write(source);
      });
    } 
...
async function tesseract(outputFileName) {
  ...
  await worker.setParameters({
    tessedit_char_whitelist: 'ABCDEFGHIJKLMNOPQRSTUVWXYZ  lIi0o|[]()',
  ...
  for(var i = 1; i<226; i++){
    const { data: { text } } = await worker.recognize('./images/slicedImages/section-' + i + '.png'); //PATH!**
    //console.log(text + " -- " + i);
